...
2. RC4 암호화 패스워드 인증
  - pass 값이 내부 키와 일치하면 세션 수립
  - RC4 스트림으로 입출력을 실시간으로 암복호화
...
| 탐지 우회        |       상세 설명          | 
| -------------------------- | ----------------- | 
| **패턴부재**           |  매직 바이트 수신 전까지 패킷 포트 로그 없음 |
| **파일 흔적 최소화** | 실행 직후 자가 삭제, 메모리만 상주하여 디스크에 남지않음 | 
| **정상 프로세스 위장**             |  SoC 기본 모니터링으로는 구별 어려움   | 
| **RC4 암호화**             | 시그니처 기반 IDS/IPS 분석 무력화  | 
...
|2| 권한 상승 및 비정상 행위 분석 | 의심스러운 프로세스 권한 상승 시도 추적|
|3|네트워크 기반 희소 패턴 헌팅|Magic Packet 수신에 따른 비정기적이고 희소한 C2 통신 흔적 식별|
