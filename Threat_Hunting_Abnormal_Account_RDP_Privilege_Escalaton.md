...
야간 시간 대, 일반 내부 사용자 계정 **Kim1234** 가 **RDP를 이용하여 10분 동안 7대의 서버에 순차적으로 로그온한 인증 로그**가 확인되었습니다. Lateral Movement 후, **3번째로 접속한 중요 서버의 DB 서버 Banking_DB** 에서 Kim1234가 Microsoft에서 제공하는 **합법적인 디버깅 도구**인 Prcodump를 사용하여 **lsass.exe** 프로세스의 메모리를 읽으려고 시도한 EDR 로그가 탐지되었습니다.

### 결론

RDP를 이용한 **Lateral Movement** 후, **Credential Dumping을 통한 추가 권한 획득** 시도로 판단

### 대응방안

* 즉각적 조치 : 해당 Kim1234 계정을 즉시 비활성화하고, 공격 경로에 포함된 모든 서버에 대한 네트워크 격리 및 포렌식 조사 진행
* 기술적 보완
  - EDR : **lsass.exe** 에 대한 비인가 프로세스의 메모리 접근을 차단하는 정책을 즉시 적용
  - SIEM : **RDP 로그온 성공 횟수**에 대한 **임계치(ex : 10분 내 3회)** 를 낮추고, 근무 외 시간 RDP 접속에 대한 고위험 경보룰 강화
  - 네트워크 : 예를들어, 금융업무관련 망과 일반 업무 망을 분리하고, RDP를 통해 접근 가능한 망을 일반 업무망으로 제한, **MFA(Multi-Factor-Authentication)** 을 적용
